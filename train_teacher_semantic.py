...
def main():
	...
	parser.add_argument('--trigger_name', type=str, default='green-car', choices=['green-car', 'racing-stripe', 'wall'])
	...
	trigger_name = "{}_t{}".format(args.trigger_name, args.target_class)

	param_string = "{}_{}_{}_e{}_{}_n{}"
	param_string = param_string.format(args.backdoor_method, args.dataset, args.model, args.epochs, trigger_name, num_poison_images)
	if args.seed != 1:
		param_string += "_seed_{}".format(args.seed)

	# output_dir = __file__.split('.')[0] + "_{}".format(param_string)
	output_dir = "train_{}".format(param_string)
	...
	backdoored_test_ds = tester.get_backdoored_test_dataset()
	...
	for epoch in range(1, args.epochs + 1):
		...
		acc = utils.test_model_acc(args, model, test_ds)
		asr = utils.test_model_acc(args, model, backdoored_test_ds)

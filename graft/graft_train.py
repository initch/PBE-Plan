...
def train(args, teacher,  
	...
	print('Train Epoch: {} [{}/{} ({:.0f}%)]\t KD_loss: {:.6f} BR_loss: {:.6f}'.format(
		epoch, epoch, 7*args.epochs, 100*float(epoch)/float(7*args.epochs), loss_S1.item(), loss_S2.item()))
...
def main():
	...
	param_string = "e_{}_npc_{}_ps_{}_wd1_{}_lrb_{}_lrn_{}_lrgp_{}"
	...
	for block_id in [1,2,3,4]:
		print(f'Grafting layer {block_id}...')
		optimizer_S, scheduler_S = graft_block(args, student, t_state_dict, block_id)
		...
		for epoch in range(1, args.epochs+1):
			...
			if args.vis_generator:
				utils.test_generators(args, {'pert':pert_generator}, args.nz2, epoch + args.epochs*(block_id-1), output_dir, plotter, norm_trans_inv=lambda x:(x+1.0)/2.0)
		...
		block_list = save_block(student, block_list, block_id)

...
class DirtyLabelPoisonedDataset(BaseOperationDataset):
	...
	def __init__(self, dataset, trigger : Trigger, num_poison_images : int, target_class : int, source_classes = None,
		...
		if sample_save_path is not None:
			poisoned_data = self.data[np.random.choice(target_img_indices, size=1, replace=False)]
			poisoned_imgs = img_grid(poisoned_data, 1, 1)[:,:,::-1]
...
class MultiTriggerPoisonedDataset(BaseOperationDataset):
	"""
		used in training DBA
	"""

	def __init__(self, dataset, trigger_list : list, num_poison_images : int, target_class : int, source_classes = None,
			 seed=0, sample_save_path=None):
		"""
			dataset : the dataset to be copied,  triggers are injected into the copied dataset.
		...
		for ind in target_img_indices:
			for trigger in self._triggers:
				self.data[ind] = trigger.paste_to_np_img(self.data[ind])
			self.targets[ind] = self._target_class

		if sample_save_path is not None:
			poisoned_data = self.data[np.random.choice(target_img_indices, size=25, replace=False)]
			poisoned_imgs = img_grid(poisoned_data, 5, 5)[:,:,::-1]
			cv2.imwrite(sample_save_path, poisoned_imgs)
